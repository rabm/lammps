...
#include "compute_rigid_ls_dem_local.h"
...
ComputeRigidLSDEMLocal::ComputeRigidLSDEMLocal(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  rstyle(nullptr), idrigid(nullptr), fixrigid(nullptr), vlocal(nullptr), alocal(nullptr)
{
  if (narg < 5) error->all(FLERR,"Illegal compute rigid/ls/dem/local command");
  ...
  for (int iarg = 4; iarg < narg; iarg++) {
    ...
    else if (strcmp(arg[iarg],"net/quatw") == 0) rstyle[nvalues++] = QUAT2W;
    else if (strcmp(arg[iarg],"net/quati") == 0) rstyle[nvalues++] = QUAT2I;
    else if (strcmp(arg[iarg],"net/quatj") == 0) rstyle[nvalues++] = QUAT2J;
    else if (strcmp(arg[iarg],"net/quatk") == 0) rstyle[nvalues++] = QUAT2K;
    ...
    else error->all(FLERR,"Invalid keyword in compute rigid/ls/dem/local command");
  }
...
ComputeRigidLSDEMLocal::~ComputeRigidLSDEMLocal()
{
...
void ComputeRigidLSDEMLocal::init()
{
  // set fixrigid

  auto *ifix = modify->get_fix_by_id(idrigid);
  if (!ifix) error->all(FLERR,"FixRigidSmallLSDEM ID {} for compute rigid/ls/dem/local does not exist", idrigid);
  fixrigid = dynamic_cast<FixRigidSmallLSDEM *>(ifix);
  if (!fixrigid)
    error->all(FLERR,"Fix ID {} for compute rigid/ls/dem/local does not point to fix rigid/ls/dem/small", idrigid);
...
void ComputeRigidLSDEMLocal::compute_local()
{
...
int ComputeRigidLSDEMLocal::compute_rigid(int flag)
{
  int i,m,n,ibody;
  double *ptr;
  FixRigidSmallLSDEM::Body *body;
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    bodyls = &fixrigid->bodyLS[ibody];

    MathExtra::quatquat(body->quat, bodyls->quatd2g, q);
    //MathExtra::qconjugate(qc, q);
...
void ComputeRigidLSDEMLocal::reallocate(int n)
{
  ...
  if (nvalues == 1) {
    memory->destroy(vlocal);
    memory->create(vlocal,nmax,"rigid/ls/dem/local:vector_local");
    vector_local = vlocal;
  } else {
    memory->destroy(alocal);
    memory->create(alocal,nmax,nvalues,"rigid/ls/dem/local:array_local");
    array_local = alocal;
  }
}
...
double ComputeRigidLSDEMLocal::memory_usage()
{
